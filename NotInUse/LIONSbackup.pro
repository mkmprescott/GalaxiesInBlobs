   ...
   FOR r=0, n_radii-1. DO BEGIN 
     ...
     print, 'GOODS-S FIELD ANALYSIS'
       ...
       FOR ap=0, nApertures-1 DO BEGIN
         ...
         ; create hstapmaglist  - will have four entries, all lists (one for each band; each entry is a list of IDs of galaxies in the aperture in each mag bin)
            hstapmaglist = list() 
            FOR i=3, 6 DO BEGIN
              maglist = list()
              FOR mag=brightestmag, dimmestmag, binsize DO BEGIN  
                hst_galaxies_binned = get_galaxies_binned(hstap_ra, hstap_dec, ap_radius[r], goodss, mag, mag+binsize, goodss.(i)) 
                maglist.Add, hst_galaxies_binned
              ENDFOR     ; all magnitude bins 
              hstapmaglist.Add, maglist
            ENDFOR   ; all bands in 3DHST 
