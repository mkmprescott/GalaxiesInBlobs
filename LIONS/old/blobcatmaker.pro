...
; INPUTS: blah                                                                                                                                    ; 
;                                                                                                                                                 ;
; new blob master catalogs will contain: ID, ra, dec, redmag, midmag, bluemag, xcolor, ycolor, class_star, imaflags_iso                           ;
;-------------------------------------------------------------------------------------------------------------------------------------------------;
   ...
   FOR blob=0, nblobs-1 DO BEGIN 
     ...
     good = WHERE((datared.CLASS_STAR LT 0.8) AND (datared.IMAFLAGS_ISO EQ 0.))
     datared = datared[good]
     datamid = datamid[good]
     datablue = datamid[good] 
     ; now make a catalog
     catname = blobname[blob] + '.csv' 
     header = ['ID', 'RA', 'DEC', 'REDMAG', 'MIDMAG', 'BLUEMAG']
     write_csv, catname, datared.NUMBER, datared.ALPHA_J2000, datared.DELTA_J2000, datared.MAG_ISO, datamid.MAG_ISO, datablue.MAG_ISO, header=header
  ENDFOR 
